classdef Clifford1 < replab.perm.PermutationBSGSGroup
    
    methods
        
        function self = Clifford1(d)
            assert(d == 2, 'Only qubits are supported');
            ...
            self = self@replab.perm.PermutationBSGSGroup(S48, {w h s}, 192);
        end
        
        function rep = quditRep(self)
            E8 = exp(1i*pi/4);
            E4 = 1i;
            W = [E8 0; 0 E8];
            H = [1 1; 1 -1]/sqrt(2);
            S = [1 0; 0 E4];
            rep = self.rep('C', 2, {W H S});
        end
