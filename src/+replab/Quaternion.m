classdef Quaternion < replab.Str
% Barebones quaternion class, defining just enough for our purposes
    ...
    methods
        function self = Quaternion(r, i, j, k)
        ...
        function qc = conj(q)
            qc = replab.Quaternion(q.r, -q.i, -q.j, -q.k);
        end
        ...
        function b = eq(q1, q2)
            q1 = replab.Quaternion(q1);
            q2 = replab.Quaternion(q2);
            b = (q1.r == q2.r) & (q1.i == q2.i) & (q1.j == q2.j) & (q1.k == q2.k);
        end
        function q = horzcat(varargin)
            arg1 = replab.Quaternion(varargin{1});
            qr = arg1.r;
            qi = arg1.i;
            qj = arg1.j;
            qk = arg1.k;
            for i = 2:nargin
                arg = replab.Quaternion(varargin{i});
                qr = horzcat(qr, arg.r);
                qi = horzcat(qi, arg.i);
                qj = horzcat(qj, arg.j);
                qk = horzcat(qk, arg.k);
            end
            q = replab.Quaternion(qr, qi, qj, qk);
        end
        function q1 = inv(q)
            assert(isscalar(q.r));
            q1 = q.conj * (1 ./ q.abs2);
        ...
        function q3 = minus(q1, q2)
            q1 = replab.Quaternion(q1);
            q2 = replab.Quaternion(q2);
            q3 = replab.Quaternion(q1.r - q2.r, q1.i - q2.i, q1.j - q2.j, q1.k - q2.k);
        end
        function q3 = mtimes(q1, q2)
            ...
            q3 = replab.Quaternion(q3r, q3i, q3j, q3k);
        end
        ...
        function q1 = subsasgn(q, s, p)
            q = replab.Quaternion(q);
            p = replab.Quaternion(p);
            qr = q.r;
            qi = q.i;
            qj = q.j;
            qk = q.k;
            ...
            qj(s.subs{:}) = p.j;
            qk(s.subs{:}) = p.k;
            q1 = replab.Quaternion(qr, qi, qj, qk);
        end
        function varargout = subsref(q, s)
            ...
            if isequal(s.type, '()')
                ...
                q1j = q.j(s.subs{:});
                q1k = q.k(s.subs{:});
                varargout{1} = replab.Quaternion(q1r, q1i, q1j, q1k);
            else
        ...
        function q1 = transpose(q)
            q1 = replab.Quaternion(q.r.', q.i.', q.j.', q.k.');
        end
        function qm = uminus(q)
            qm = replab.Quaternion(-q.r, -q.i, -q.j, -q.k);
        end
        function qp = uplus(q)
            qp = q;
        end
