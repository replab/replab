function elements = CompleteDimino_permutations(generators, group)
...
% elements in the S3 permutation group
    ...
    ds = group.domainSize;
    elements = replab.perm.Set(ds, ds);
    identity = 1:ds;
    elements.insert(identity');
    ...
    for i = 2:length(generators)
        ...
        for e = 1:elements.nElements
            if all(elements.at(e)' == s)
                not_redundant = 0;
                break
            end
        end
        if not_redundant
            ...
            for j = 1:(i-1) % generators H (current subgroup) are s1 to s(i-1)
                ...
                ind = elements.find(conjugate');

                for k = 2:length(elements)
                    if all(elements{k} == conjugate)
                        break
                    end
                end
            ...
            for j = 2:prev_order
                order = order + 1;
                elements.insert(group.compose(elements{j}, s)');
            end
