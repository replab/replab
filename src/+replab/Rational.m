...
%
% The implementation below assumes that coefficients stay small, as they are stored in standard floating-point numbers,
% it will fail silently when the coefficient complexity grows too large.

    properties
        N = []; % (integer(\*,\*)): Numerator part
        d = 1; % (integer): Denominator common to all matrix elements
    end
    ...
    methods
        ...
        function R = pinv(R)
            ...
            INV = R.d./replab.Rational(R.N(R.N~=0));
            R.N(R.N~=0) = INV.N;
            R.N = R.N';
            R.d = INV.d;
        end
        ...
        function R = simplify(R)
            Ngcd = replab.Rational.matgcd(R.N);
            g = gcd(Ngcd, R.d);
            if g ~= 1
                R.N = R.N ./ g;
                R.d = R.d / g;
        ...
        function R1 = diag(R)
            R1 = replab.Rational(diag(R.N), R.d);
        end
        ...
        function R = mtimes(A, B)
            if isa(A, 'sym') || isa(B, 'sym')
                ...
                A = replab.Rational(A);
                B = replab.Rational(B);
                R = simplify(replab.Rational(A.N * B.N, A.d * B.d));
            end
        end
        ...
        % TODO: what does this do?
            ...
            Nres(N < 0) = -1;
            N = abs(N);
            f = unique(factor(abs(replab.Rational.matlcm(N))));
            for i = f
                while gcd(replab.Rational.matlcm(N), i*i) == i*i && i > 1
                    ind = (mod(N, i*i) == 0) & (N ~= 0);
                    N(ind) = N(ind) / (i*i);
                    Nroot(ind) = Nroot(ind) * i;
                end
                while gcd(replab.Rational.matlcm(N), i) == i && i > 1
                    ind = (mod(N, i) == 0) & (N ~= 0);
                    N(ind) = N(ind) / i;
                    Nres(ind) = Nres(ind) * i;
                end
            end
            root = replab.Rational(Nroot, droot);
            res = replab.Rational(Nres, dres);
        end

        function R = power(R1, n)
            R = replab.Rational(R1.N.^n, R1.d.^n);
        end
        ...
        function R1 = sum(R, varargin)
            R1 = replab.Rational(sum(R.N, varargin{:}), R.d);
        end
        ...
        function R = horzcat(A, B, varargin)
            ...
            [A, B, d] = common(A, B);
            R = simplify(replab.Rational([A.N B.N],d));
        end
        ...
        function R = kron(A, B)
            A = replab.Rational(A);
            B = replab.Rational(B);
            R = simplify(replab.Rational(kron(A.N, B.N), A.d * B.d));
        end
        ...
        function R1 = subsref(R, S)
            switch S(1).type
              ...
              case '()'
                R1 = simplify(replab.Rational(subsref(R.N, S), R.d));
              otherwise
                error('');
            end
        end
        ...
        function g = matgcd(M, filtered)
            ...
            switch n
              ...
              case 2
                g = gcd(g, gcd(M(1), M(2)));
              otherwise
                g = gcd(g, replab.Rational.matgcd(gcd(M(1:n/2), M(n/2+1:n)), true));
            end
        end
        ...
        function l = matlcm(M, filtered)
            ...
            switch n
              ...
              case 2
                l = lcm(l, lcm(M(1), M(2)));
              otherwise
                l = lcm(l, replab.Rational.matlcm(lcm(M(1:n/2), M(n/2+1:n)), true));
            end
        end
