...
%         for k = nclasses - 1:nclasses
%             chars(2, k) = replab.cyclotomic.fromDoubles(-1);
%             chars(3, k) = replab.cyclotomic.fromDoubles((-1)^k);
%             chars(4, k) = replab.cyclotomic.fromDoubles((-1)^(k+1));
%             for j = 1:stop
%                 chars(n1D+j, k) = replab.cyclotomic.fromDoubles(0);
%             end
%         end
    else
        for k = 1:nclasses - 2
